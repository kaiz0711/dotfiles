...
async function downloadServer(context, release) {
  ...
  const resp = await lib_default(release.url, {headers: {"user-agent": "VSCode"}});
  if (!resp.ok) {
    statusItem.hide();
...
var Ctx2 = class {
  ...
  resolveBin() {
    ...
    const args = ["-E", import_path3.default.join(serverDir, "main.lua"), `--locale=${this.config.locale}`].concat(import_coc5.workspace.getConfiguration("Lua").get("misc.parameters"));
